...
export default function LogsLoginPage() {
  ...
  useEffect(() => {
    if (initializedDates) return;

    const hoje = new Date();
    const umDiaAntes = new Date(hoje);
    umDiaAntes.setDate(hoje.getDate() - 1);

    const defaultFrom = `${umDiaAntes.getFullYear()}-${String(
      umDiaAntes.getMonth() + 1,
    ).padStart(2, "0")}-${String(umDiaAntes.getDate()).padStart(2, "0")}`;

    const defaultTo = `${hoje.getFullYear()}-${String(
      hoje.getMonth() + 1,
